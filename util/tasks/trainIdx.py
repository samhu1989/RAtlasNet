...
def train_ae(net,optim,cd_meter,inv_meter,pts,opt):
    ...
    with torch.no_grad():
        out = net(points);
    index = out['idx'](points);
    y = torch.bmm(out['y'].transpose(2,1).contiguous(),index);
    v,_ = torch.max(index,dim=1);
    cd = 2.0*torch.mean(torch.sum(( y - points )**2,dim=1));
    inv_err = torch.mean(torch.sum((out['inv_x'] - out['grid_x'])**2,dim=2));
    cd_meter.update(cd.data.cpu().numpy());
    inv_meter.update(inv_err.data.cpu().numpy())
    if cd < opt['w']*inv_err:
    	loss = cd + opt['w']*inv_err + 0.001*torch.mean(v);
    else:
        loss = cd + 0.001*torch.mean(v);
    loss.backward();
    optim.step();
...
class RealTask(Task):
    ...
    def start(self):
        ...
        self.optim = optim.Adam(self.net.idx_net.parameters(),lr=self.opt['lr'],weight_decay=self.opt['weight_decay']);
